  ...
  return (
    <div>
      <PageHero
        ...
        description="Okulumuzda düzenlenen geziler, atölyeler ve özel programlar."
      />
      <div className="mx-auto max-w-6xl space-y-6 px-4 py-12 sm:space-y-8 sm:px-5 sm:py-16 md:px-8">
        ...
        {events.map((event) => (
          ...
          >
            {event.coverImage && (
              <div className="relative aspect-[16/9] bg-navy/5 sm:aspect-[21/9]">
                <Image
              ...
              {event.categories.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {event.categories.map((category) => (
                    <span
                      key={category.id}
                      className="bg-navy/8 px-2 py-0.5 text-xs font-medium text-navy"
                    >
                      {category.name}
                    </span>
                  ))}
